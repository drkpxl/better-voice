...
#
# 用户视角全链路：用一段已知音频跑 WE --bench-voice（走完整 ContextEnhancer +
# SpeechAnalyzer + L2 polish 链路，但不注入光标），校验：
#   (a) WE --bench-voice 进程能正常退出（无超时）
...
# 找 WE 二进制（优先 .app bundle，回退 debug）
    ...
    cat <<EOF
{
  ...
  "score": 0,
  "evidence": null,
  "note": "WE binary not found; run 'cd client && make build' first"
}
EOF
    exit 0
fi
...
exit_code=0
perl -e 'alarm(60); exec @ARGV' -- "$WE_BIN" --bench-voice "$TEST_AUDIO" --output "$RESULT_JSON" >/dev/null 2>&1 || exit_code=$?
