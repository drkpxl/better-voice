...
final class WavHeaderTests: XCTestCase {
    ...
    func test_headerIsCanonical44BytePCM() {
        ...
        let h = makeWavHeader(sampleRate: 16000, channels: 1, bitsPerSample: 16, dataSize: 1000, isFloat: false)
    ...
    func test_stereo24bitDerivedFields() {
        let h = makeWavHeader(sampleRate: 48000, channels: 2, bitsPerSample: 24, dataSize: 0, isFloat: false)
        XCTAssertEqual(u16(h, 32), 6)                 // blockAlign = 2 * (24/8)
        XCTAssertEqual(u32(h, 28), 48000 * 6)         // byteRate
        XCTAssertEqual(u32(h, 4), 36)                 // 36 + 0
    }

    /// Regression guard for a real corruption bug found while implementing mic+system meeting
    /// capture: Core Audio process taps deliver Float32 samples, but `makeWavHeader` used to
    /// hardcode format tag 1 (WAVE_FORMAT_PCM / integer) unconditionally. `AVAudioFile` decodes
    /// the format tag literally — tag 1 + float32 bytes silently produces garbage samples, not
    /// an error (verified empirically: values round-trip exactly under tag 3, are corrupted
    /// under tag 1). Every meeting recording captured via `SystemAudioCapturer`/`MicCapturer`
    /// needs tag 3.
    func test_isFloatWritesIEEEFloatFormatTag() {
        let h = makeWavHeader(sampleRate: 48000, channels: 1, bitsPerSample: 32, dataSize: 400, isFloat: true)
        XCTAssertEqual(u16(h, 20), 3)                 // WAVE_FORMAT_IEEE_FLOAT
        XCTAssertEqual(u16(h, 34), 32)                // bits per sample unaffected
    }

    func test_isFloatFalseWritesIntegerPCMFormatTag() {
        let h = makeWavHeader(sampleRate: 48000, channels: 1, bitsPerSample: 32, dataSize: 400, isFloat: false)
        XCTAssertEqual(u16(h, 20), 1)                 // WAVE_FORMAT_PCM
    }
}
