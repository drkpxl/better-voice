...
///
/// Single implementation shared by `SystemAudioCapturer` and `MicCapturer` — the two-file
/// meeting capture's system and mic WAVs respectively — which previously carried byte-for-byte
/// copies of this logic. Opens lazily on the first
...
final class PCMWavWriter {
    ...
    func finalize() {
        ...
        // mic buffers (MicCapturer) can be Float32 or Int16 depending on the input device — this
        // isn't hardcoded to either. The header's format tag MUST match the actual sample
        // encoding — see `makeWavHeader`'s doc comment for why getting this wrong silently
        // corrupts every sample instead of failing loudly.
