...
enum TextInjector {
    ...
    static func inject(text: String, to app: AppIdentity?, focusTarget: AXUIElement? = nil) {
        ...
        // Wait for the keyboard's modifier keys to be RELEASED before synthesizing ⌘V.
        //
        // The old dictation hotkey was modifier-only and fired on the release edge, so by the time
        // we got here the keyboard was always quiescent. The ⌥. *combo* hotkey fires on key-DOWN,
        // so whether Option is still held when we post — a race that depends on how fast the user
        // lifted the key — determines whether the target sees a clean ⌘V or a ⌘⌥V that pastes
        // nothing. Terminals in particular read the live/global modifier state, so even an event
        // whose own flags say "Command only" is reinterpreted with a still-held Option. Reproduce
        // the old guarantee explicitly: poll until no command/option/shift/control is down (capped
        // so a user who genuinely holds the key can't hang injection), then post a clean ⌘V.
        let heldModifiers: CGEventFlags = [.maskCommand, .maskAlternate, .maskShift, .maskControl]
        var waitedUs = 0
        while waitedUs < 500_000,
              !CGEventSource.flagsState(.combinedSessionState).intersection(heldModifiers).isEmpty {
            usleep(15_000)
            waitedUs += 15_000
        }

        // Simulate ⌘V — posted to cgSessionEventTap (more reliable than cghidEventTap on macOS 14+).
        // `.combinedSessionState` is the proven-reliable delivery source; with modifiers now
        // confirmed clear above, it can't contaminate the paste with a stray flag.
